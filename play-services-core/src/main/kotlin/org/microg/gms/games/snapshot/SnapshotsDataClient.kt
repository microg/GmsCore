...
class SnapshotsDataClient(val context: Context) {
    ...
    suspend fun loadSnapshotData(packageName: String, account: Account) = withContext(Dispatchers.IO) {
        val token = GoogleAuthUtil.getToken(context, account, Games.SERVICE_GAMES_LITE, packageName)
        val snapshots = arrayListOf<Snapshot>()
        val snapshotsResponse = runCatching { SnapshotsApiClient.requestSnapshotList(context, token) }.getOrNull()
        snapshotsResponse?.gameSnapshot?.forEach {
            snapshots.add(
                Snapshot(
                    it.metadata?.snapshot?.snapshotId,
                    it.metadata?.snapshotName,
                    it.metadata?.snapshot?.content?.description,
                    it.metadata?.snapshot?.content?.snapshotTimeInfo?.timestamp?.toString(),
                    SnapshotImage(
                        it.metadata?.snapshot?.coverImage?.width,
                        it.metadata?.snapshot?.coverImage?.height,
                        it.metadata?.snapshot?.coverImage?.mimeType,
                        it.metadata?.snapshot?.coverImage?.imageUrl,
                        null
                    )
                )
            )
    ...
    private suspend fun uploadDataByUrl(packageName: String,account: Account, url: String, body: ByteArray, oauthService: String) = withContext(Dispatchers.IO) {
        val token = GoogleAuthUtil.getToken(context, account, oauthService, packageName)
        runCatching { SnapshotsApiClient.uploadDataByUrl(token, url, requestQueue, body) }
    }

    suspend fun getDataFromDrive(packageName: String, account: Account, url: String, oauthService: String) = withContext(Dispatchers.IO) {
        val token = GoogleAuthUtil.getToken(context, account, oauthService, packageName)
        runCatching { SnapshotsApiClient.getDataFromDrive(token, url, requestQueue) }.getOrNull()
    }

    private suspend fun getRealUploadUrl(packageName: String, account: Account, url: String, oauthService: String) = withContext(Dispatchers.IO) {
        val token = GoogleAuthUtil.getToken(context, account, oauthService, packageName)
        runCatching { SnapshotsApiClient.getRealUploadUrl(token, url, requestQueue) }.getOrNull()
    }

    suspend fun resolveSnapshotHead(packageName: String, account: Account, resolveSnapshotHeadRequest: ResolveSnapshotHeadRequest, oauthService: String) = withContext(Dispatchers.IO) {
        val token = GoogleAuthUtil.getToken(context, account, oauthService, packageName)
        runCatching { SnapshotsApiClient.resolveSnapshotHead(context, token, resolveSnapshotHeadRequest) }.getOrNull()
    }

    private suspend fun prepareSnapshotRevision(packageName: String, account: Account, oauthService: String, prepareSnapshotRevisionRequest: PrepareSnapshotRevisionRequest) = withContext(Dispatchers.IO) {
        val token = GoogleAuthUtil.getToken(context, account, oauthService, packageName)
        runCatching { SnapshotsApiClient.prepareSnapshotRevision(context, token, prepareSnapshotRevisionRequest) }.getOrNull()
    }

    private suspend fun commitSnapshotRevision(packageName: String, account: Account, commitSnapshotRevisionRequest: CommitSnapshotRevisionRequest, oauthService: String) = withContext(Dispatchers.IO) {
        val token = GoogleAuthUtil.getToken(context, account, oauthService, packageName)
        runCatching { SnapshotsApiClient.commitSnapshotRevision(context, token, commitSnapshotRevisionRequest) }.getOrNull()
    }

    suspend fun deleteSnapshotData(packageName: String, account: Account, snapshot: Snapshot) = withContext(Dispatchers.IO) {
        val token = GoogleAuthUtil.getToken(context, account, Games.SERVICE_GAMES_LITE, packageName)
        runCatching { SnapshotsApiClient.deleteSnapshot(context, token, snapshot) }.getOrNull()
    }


    suspend fun commitSnapshot(packageName: String, account: Account, snapshotTitle: String?, oauthService: String, change: SnapshotMetadataChangeEntity,
                               contents: Contents, maxCoverImageSize: Int) = withContext(Dispatchers.IO) {
        runCatching {
            ...
            val ret = prepareSnapshotRevision(packageName, account, oauthService, createPrepareSnapshotRevisionRequest(change, snapshotTitle!!))
            ...
            if (ret != null && ret.uploadLinkInfos.isNotEmpty()) {
                ...
                val deferredGetImageRealUploadUrl = async {
                    ...
                    if (imageUploadTempUrl == null) {
                        ...
                        getRealUploadUrl(packageName, account, imageUploadTempUrl, oauthService)
                    }
                }
                val deferredGetDataRealUploadUrl = async {
                    getRealUploadUrl(packageName, account, snapshotDataUploadTempUrl, oauthService)
                }
                ...
                val deferredUploadSnapshotImage = async {
                    ...
                    if (snapshotImageUploadUrl == null) {
                        ...
                        uploadSnapshotImage(change!!.bitmapTeleporter!!.createTargetBitmap()
                            , maxCoverImageSize, packageName, account, oauthService, snapshotImageUploadUrl)
                    }
                ...
                val deferredUploadSnapshotData = async {
                    ...
                    uploadSnapshotData(contents, packageName, account, snapshotDataUploadUrl, oauthService)
                }
                ...
                val deferredCommit =  async {
                    ...
                    if (snapshotDataResourceId != null) {
                        commitSnapshotRevision(packageName, account, createCommitSnapshotRequest(change, snapshotDataResourceId,
                            snapShotImageResourceId, imageWidth, imageHeight, snapshotTitle), oauthService)
                    } else {
                        return@async false
                    }
                }
                ...
                Log.w(TAG, "commitSnapshot commit finish")
            } else {
                return@runCatching false
            }
        }.getOrNull()
    ...
    private fun createCommitSnapshotRequest(change: SnapshotMetadataChangeEntity, snapshotDataResourceId: String,
                                                    snapShotImageResourceId: String?, imageWidth: Int, imageHeight: Int, snapshotTitle: String) : CommitSnapshotRevisionRequest {
    ...
    private suspend fun uploadSnapshotData(contents: Contents, packageName: String, account: Account, snapshotDataUploadUrl: String, oauthService: String) : String? {
        ...
        try {
            ...
            readInputStreamFully =
                IOUtils.readInputStreamFully(bufferedInputStream, false)
            fileInputStream.channel.position(0L)
            snapshotDataResourceId = uploadDataByUrl(packageName,
                account, snapshotDataUploadUrl, readInputStreamFully, oauthService).getOrNull()
    ...
    private suspend fun uploadSnapshotImage(bitmap:Bitmap, maxCoverImageSize: Int, packageName: String,
                                            account: Account, oauthService: String, imageUploadUrl: String) : Triple<Int, Int, String?> {
        ...
        val snapShotImageResourceId = uploadDataByUrl(packageName, account
            , imageUploadUrl, byteArrayOutputStream.toByteArray(), oauthService).getOrNull()
