    ...
    override fun handleServiceRequest(callback: IGmsCallbacks, request: GetServiceRequest, service: GmsService) {
        ...
        lifecycleScope.launchWhenStarted {
            try {
                ...
                val oauth2Service = "oauth2:${scopes.joinToString(" ")}"
                val authManager = AuthManager(this@GamesService, account.name, packageName, oauth2Service)
                ...
                callback.onPostInitCompleteWithConnectionInfo(
                    ...
                    GamesServiceImpl(this@GamesService, lifecycle, packageName, account, player, oauth2Service),
                    ConnectionInfo()
                )
            } catch (e: Exception) {
                Log.w(TAG, e)
...
class GamesServiceImpl(val context: Context, override val lifecycle: Lifecycle, val packageName: String, val account: Account, val player: Player, val oauthService:String) :
    ...
    override fun clientDisconnecting(clientId: Long) {
        Log.d(TAG, "Method clientDisconnecting called:$clientId")
        AchievementsDataClient.get(context).release()
    }
    ...
    override fun loadAchievements(callbacks: IGamesCallbacks?) {
        Log.d(TAG, "Method loadAchievements called")
    ...
    override fun revealAchievement(callbacks: IGamesCallbacks?, achievementId: String, windowToken: IBinder?, extraArgs: Bundle?) {
        ...
        lifecycleScope.launchWhenStarted {
            runCatching {
                val ret = AchievementsDataClient.get(context).revealAchievement(account, packageName, achievementId)
                callbacks?.onAchievementUpdated(if (ret != -1) 0 else ret, achievementId)
            }.onFailure {
    ...
    override fun unlockAchievement(callbacks: IGamesCallbacks?, achievementId: String, windowToken: IBinder?, extraArgs: Bundle?) {
        ...
        lifecycleScope.launchWhenStarted {
            runCatching {
                val ret = AchievementsDataClient.get(context).unlockAchievement(account, packageName, achievementId)
                callbacks?.onAchievementUpdated(if (ret != -1) 0 else ret, achievementId)
            }.onFailure {
    ...
    override fun incrementAchievement(callbacks: IGamesCallbacks?, achievementId: String, numSteps: Int, windowToken: IBinder?, extraArgs: Bundle?) {
        ...
        lifecycleScope.launchWhenStarted {
            runCatching {
                val ret = AchievementsDataClient.get(context).incrementAchievement(account, packageName, achievementId, numSteps)
                callbacks?.onAchievementUpdated(if (ret != -1) 0 else ret, achievementId)
            }.onFailure {
    ...
    override fun loadAchievementsV2(callbacks: IGamesCallbacks?, forceReload: Boolean) {
        Log.d(TAG, "Method loadAchievementsV2 called: forceReload:$forceReload")
        lifecycleScope.launchWhenStarted {
            AchievementsDataClient.get(context).loadAchievementsData(packageName, account, forceReload)?.run {
                callbacks?.onAchievementsLoaded(this)
            }
        }
    }
    ...
    override fun setAchievementSteps(callbacks: IGamesCallbacks?, achievementId: String, numSteps: Int, windowToken: IBinder?, extras: Bundle?) {
        ...
        lifecycleScope.launchWhenStarted {
            runCatching {
                val ret = AchievementsDataClient.get(context).setAchievementSteps(account, packageName, achievementId, numSteps)
                callbacks?.onAchievementUpdated(if (ret != -1) 0 else ret, achievementId)
            }.onFailure {
    ...
    override fun commitSnapshot(
        ...
        lifecycleScope.launchWhenStarted {
            if (change != null && contents?.parcelFileDescriptor != null) {
                runCatching {
                    val result = SnapshotsDataClient.get(context)
                        .commitSnapshot(packageName, account, saveName, oauthService, change, contents, maxCoverImageSize)
                    if (result == true) {
    ...
    override fun resolveSnapshotHead(callbacks: IGamesCallbacks, saveName: String?, i: Int) {
        Log.d(TAG, "Method saveSnapshot $saveName, $i")
        if (TextUtils.isEmpty(saveName)) {
            Log.w(TAG, "saveSnapshot: Must provide a non empty fileName!")
            return
        }
        if (!pattern.matcher(saveName).matches()) {
            Log.w(TAG, "saveSnapshot: Must provide a valid file name!")
            return
        }
        ...
        lifecycleScope.launchWhenStarted {
            ...
            val resolveSnapshotHeadResponse = SnapshotsDataClient.get(context).resolveSnapshotHead(packageName, account, resolveSnapshotHeadRequest, oauthService)
            val contentUrl = resolveSnapshotHeadResponse?.snapshotMetadata?.snapshot?.snapshotContentInfo?.url
            if (contentUrl != null) {
                val contentByteArray = SnapshotsDataClient.get(context).getDataFromDrive(packageName, account, contentUrl, oauthService)
    ...
    override fun getCurrentAccount(): Account? {
        Log.d(TAG, "Method getCurrentAccount Called: ${account.name}")
        return account
    }
