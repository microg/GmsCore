    ...
    override fun onBindViewHolder(holder: AchievementsHolder, position: Int) {
        ...
        if (definition.type == -1) {
            ...
            holder.achievementContent?.text = mContext.getString(
                ...
                definition.xpValue.toString()
            )
            val imageUrl =
                if (definition.state == Achievement.AchievementState.STATE_UNLOCKED) {
                    definition.unlockedImageUrl
                } else {
                    definition.revealedImageUrl
                }
            if (imageUrl != null) {
                ImageManager.create(mContext).loadImage(imageUrl, holder.achievementLogo)
            } else {
                val logoId = if (definition.state == Achievement.AchievementState.STATE_UNLOCKED) {
