...
fun JSONObject.toListLeaderboardScoresResponse() = ListLeaderboardScoresResponse(
    ...
    optJSONArray("items")?.toLeaderboardEntryList(),
)

fun JSONArray.toLeaderboardEntryList(): List<LeaderboardEntry> {
...
fun String.toLeaderboardOrder(): Int {
    if (equals("LARGER_IS_BETTER")) return Leaderboard.SCORE_ORDER_LARGER_IS_BETTER
    if (equals("SMALLER_IS_BETTER")) return Leaderboard.SCORE_ORDER_SMALLER_IS_BETTER
    return -1
}
