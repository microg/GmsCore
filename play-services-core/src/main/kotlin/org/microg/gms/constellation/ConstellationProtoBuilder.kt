...
/**
 * Build SetConsentRequest proto.
 */
fun buildSetConsentRequest(
    sessionId: String,
    ctx: RequestProtoContext,
    setConsentToken: String?
): SetConsentRequest {
    val clientInfo = buildClientInfo(
        ctx = ctx,
        droidGuardToken = setConsentToken
    )
    return SetConsentRequest(
        ...
        asterism_client = AsterismClient.ASTERISM_CLIENT_UNKNOWN
    )
}
...
fun bundleToParams(bundle: Bundle?): List<Param> {
    ...
    return bundle.keySet().mapNotNull { key ->
        val value = bundle.getString(key) ?: return@mapNotNull null
        Param(name = key, value_ = value)
    }
}

/**
 * Build a CarrierInfo proto for the Sync/Proceed requests.
 */
