...
private suspend fun executeProceed(
    ...
    try {
        val proceedResponse = requestContext.rpc.proceed(proceedRequest)
        val nextVerification = proceedResponse.verification
        if (nextVerification == null) {
            Log.w(TAG, "Round $round: proceed response verification missing")
            return ProceedExecutionOutcome.Error("proceed-verification-missing")
        }
        val nextState = nextVerification.state
        Log.i(TAG, "Round $round: proceed response received. state=$nextState")

        return when (nextState) {
            VerificationState.VERIFICATION_STATE_VERIFIED -> ProceedExecutionOutcome.Verified(nextVerification)
            VerificationState.VERIFICATION_STATE_PENDING -> ProceedExecutionOutcome.Pending(nextVerification)
            else -> ProceedExecutionOutcome.OtherState(nextState)
        }
    } catch (e: Exception) {
        if (e is com.squareup.wire.GrpcException) {
            Log.e(TAG, "Round $round: proceed gRPC error: code=${e.grpcStatus.code} message=${e.grpcMessage}")
            if (e.grpcStatus.code == 7 || e.grpcStatus.code == 16) {
                requestContext.rpc.clearDroidGuardTokenCache(
                    requestContext.rpc.resolveDroidGuardFlow("proceed"),
                    "Auth error (grpc-status=${e.grpcStatus.code})"
                )
            }
        } else {
            Log.e(TAG, "Round $round: proceed request failed: ${e.javaClass.simpleName}: ${e.message}")
        }
        return ProceedExecutionOutcome.Error("proceed-failed", e)
    }
}
