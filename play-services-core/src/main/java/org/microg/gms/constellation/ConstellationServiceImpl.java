...
public class ConstellationServiceImpl extends IConstellationApiService.Stub {
    ...
    public void verifyPhoneNumber(IConstellationCallbacks callbacks, VerifyPhoneNumberRequest request, ApiMetadata metadata) throws RemoteException {
        ...
        worker.execute(() -> {
            try {
                // Correct extraction based on VerifyPhoneNumberRequest structure (policyId is UPI policy, not phone).
                String phoneNumber = null;
                int subId = -1;

                if (request != null) {
                    if (request.imsiRequests != null && !request.imsiRequests.isEmpty()) {
                        ImsiRequest imsiReq = request.imsiRequests.get(0);
                        if (imsiReq != null && imsiReq.msisdn != null && imsiReq.msisdn.startsWith("+")) {
                            phoneNumber = imsiReq.msisdn;
                        }
                    }
                    Bundle extras = request.extras;
                    if (extras != null) {
                        if (phoneNumber == null) {
                            phoneNumber = extras.getString("phone_number");
                            if (phoneNumber == null) phoneNumber = extras.getString("phoneNumber");
                            if (phoneNumber == null) phoneNumber = extras.getString("msisdn");
                            if (phoneNumber == null) phoneNumber = extras.getString("number");
                        }
                        if (subId <= 0) {
                            subId = extras.getInt("subscription_id", extras.getInt("subscriptionId", extras.getInt("sub_id", -1)));
                            if (subId == -1) {
                                subId = (int) extras.getLong("subscription_id", extras.getLong("subscriptionId", extras.getLong("sub_id", -1L)));
                            }
                        }
                    }
                    if ((phoneNumber == null || phoneNumber.isEmpty() || !phoneNumber.startsWith("+")) &&
                        request.policyId != null && request.policyId.startsWith("+")) {
                        phoneNumber = request.policyId;
                    }
                }
