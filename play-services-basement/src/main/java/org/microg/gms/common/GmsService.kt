...
enum class GmsService(var SERVICE_ID: Int, vararg actions: String) {
    UNKNOWN(-2),
    ANY(-1),
    ...
    @JvmField
    var ACTION: String? = if (actions.isNotEmpty()) actions[0] else null
    var SECONDARY_ACTIONS: Array<String> = actions.asList().toTypedArray()

    interface ADVERTISING_ID {
        companion object {
            // Has no service id
            const val ACTION = "com.google.android.gms.ads.identifier.service.START"
        }
    }

    fun toString(serviceId: Int): String =
            if (this != UNKNOWN) toString() else "UNKNOWN($serviceId)"

    companion object {
        @JvmStatic
        fun byServiceId(serviceId: Int): GmsService {
        ...
        fun byAction(action: String): GmsService {
            for (service in values()) {
                for (serviceAction in service.SECONDARY_ACTIONS) {
                    if (serviceAction == action) return service
                }
            }
            return UNKNOWN
        }
        ...
        @JvmStatic
        fun nameFromServiceId(serviceId: Int): String {
