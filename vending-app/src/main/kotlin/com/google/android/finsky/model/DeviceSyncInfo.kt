...
import com.google.android.finsky.AccountAssValue
import com.google.android.finsky.AccountAssociationPayload
import com.google.android.finsky.CarrierPropertiesPayload
import com.google.android.finsky.DeviceAccountsPayload
import com.google.android.finsky.DeviceCapabilitiesPayload
import com.google.android.finsky.DeviceInfoCollect
import com.google.android.finsky.DeviceInputPropertiesPayload
import com.google.android.finsky.DeviceModelPayload
import com.google.android.finsky.EnterprisePropertiesPayload
import com.google.android.finsky.FeatureInfoProto
import com.google.android.finsky.GpuInfoWrapper
import com.google.android.finsky.GpuPayload
import com.google.android.finsky.HardwareIdentifierPayload
import com.google.android.finsky.HardwarePropertiesPayload
import com.google.android.finsky.LocalePropertiesPayload
import com.google.android.finsky.MangedScope
import com.google.android.finsky.PlayPartnerPropertiesPayload
import com.google.android.finsky.PlayPropertiesPayload
import com.google.android.finsky.ProfileInfo
import com.google.android.finsky.ScreenPropertiesPayload
import com.google.android.finsky.SyncReqWrapper
import com.google.android.finsky.SyncRequest
import com.google.android.finsky.SystemPropertiesPayload
import com.google.android.finsky.TelephonyInfo
import com.google.android.finsky.TelephonyStateWrapper
...
object DeviceSyncInfo {
    ...
    private fun buildPayloads(context: Context, androidId: String, account: Account): Array<SyncRequest?> {
        ...
        return arrayOf(accountAssociationPayloadRequest, carrierPropertiesPayloadRequest, deviceAccountsPayloadRequest,
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                ...
                val maxVersion = infos.stream()
                    .max(Comparator.comparingInt {
                        fetchedGlStrings: FetchedGlStrings ->
    ...
    private fun createHardwareIdentifierPayloadRequest(context: Context): SyncRequest? {
        ...
        try {
            ...
            var imeid: Long = if (TextUtils.isEmpty(randomIMEI) || !Pattern.compile("^[0-9]{15}$").matcher(randomIMEI).matches()) 0L else randomIMEI.toLong(10) or 0x1000000000000000L
            if (imeid == 0L) {
                var meid = ""
                ...
                if (!TextUtils.isEmpty(meid) && Pattern.compile("^[0-9a-fA-F]{14}$").matcher(meid).matches()) {
                    ...
                    if (imeid == 0L) {
                        if (context.packageManager.checkPermission("android.permission.READ_PRIVILEGED_PHONE_STATE", "com.android.vending") == PackageManager.PERMISSION_GRANTED) {
                            var serial = ""
                            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
        ...
        try {
            ...
            if (activeAdmins != null) {
                for (componentName in activeAdmins) {
                    ...
                    val policyType = if (isDeviceOwner) MangedScope.MANAGED_DEVICES else if (isProfileOwner) MangedScope.MANAGED_PROFILES else MangedScope.LEGACY_DEVICE_ADMINS
    ...
    private fun createDeviceCapabilitiesPayloadRequest(deviceInfoCollect: DeviceInfoCollect): SyncRequest {
        ...
        for (featureInfoProto in deviceInfoCollect.featureInfoList) {
            featureInfoList.add(FeatureInfoProto.Builder()
        ...
        try {
            ...
            for (v1 in 0 until configCount) {
                if (egl10Instance?.eglGetConfigAttrib(eglDisplay, eglConfigs?.get(v1), 0x3027) != 0x3050
                    ...
                    if ((v2?.and(4)) != 0) {
                        egl10Instance?.let { wrapper -> buildGLStrings(wrapper, eglDisplay, eglConfigs?.get(v1), arrV1, intArrayOf(0x3098, 2, 0x3038))?.let { result.add(it) } }
                    }
                }
            }
    ...
    internal class FetchedGlStrings(var contextClientVersion: Int,
