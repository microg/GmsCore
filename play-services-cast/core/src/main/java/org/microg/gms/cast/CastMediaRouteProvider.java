...
public class CastMediaRouteProvider extends MediaRouteProvider {
    ...
    // ConcurrentHashMap prevents ConcurrentModificationException when NSD resolution
    // callbacks add/remove devices while the main thread iterates in publishRoutes().
    private final Map<String, CastDevice> castDevices = new ConcurrentHashMap<>();
    private final Map<String, String> serviceCastIds = new ConcurrentHashMap<>();
    ...
    static {
        ...
        String[] types = {
                "image/jpeg", "image/pjpeg", "image/jpg", "image/webp", "image/png",
                "image/gif", "image/bmp", "image/vnd.microsoft.icon",
                "image/x-icon", "image/x-xbitmap",
                "audio/wav", "audio/x-wav", "audio/mp3", "audio/x-mp3", "audio/x-m4a",
                "audio/mpeg", "audio/webm", "audio/ogg", "audio/x-matroska",
    ...
    public CastMediaRouteProvider(Context context) {
        ...
        mNsdManager = (NsdManager) context.getApplicationContext()
        ...
        mDiscoveryListener = new NsdManager.DiscoveryListener() {
                ...
                mNsdManager.resolveService(service, new NsdManager.ResolveListener() {
                    ...
                    public void onResolveFailed(NsdServiceInfo serviceInfo, int errorCode) {
                        if (errorCode == NsdManager.FAILURE_ALREADY_ACTIVE) {
                            // Resolution already in progress for this service; ignore.
                            return;
                        }
                    ...
                    public void onServiceResolved(NsdServiceInfo serviceInfo) {
                        ...
                        if (attributes == null) {
                            Log.e(TAG, "Missing DNS-SD attributes");
                            return;
                        }
                        try {
                            ...
                            int status = Integer.parseInt(
                                    new String(attributes.get("st"), "UTF-8"));
                            ...
                            Log.e(TAG, "Error parsing DNS-SD response", ex);
                        }
                    }
                });
            }
            ...
            public void onServiceLost(NsdServiceInfo serviceInfo) {
                onChromeCastLost(serviceInfo.getServiceName());
            }
    ...
    private void onChromeCastDiscovered(String id, String name, InetAddress host, int port,
            String deviceVersion, String friendlyName, String modelName,
            String iconPath, int status) {
        if (!castDevices.containsKey(id)) {
            int capabilities = CastDevice.CAPABILITY_VIDEO_OUT | CastDevice.CAPABILITY_AUDIO_OUT;
            CastDevice castDevice = new CastDevice(id, name, host, port, deviceVersion,
                    friendlyName, modelName, iconPath, status, capabilities);
            castDevices.put(id, castDevice);
    ...
    public void onDiscoveryRequestChanged(MediaRouteDiscoveryRequest request) {
        ...
        if (request != null && request.isValid() && request.isActiveScan()) {
            ...
            if (state == State.NOT_DISCOVERING) {
                mNsdManager.discoverServices(
                        "_googlecast._tcp.", NsdManager.PROTOCOL_DNS_SD, mDiscoveryListener);
    ...
    public RouteController onCreateRouteController(String routeId) {
        CastDevice castDevice = castDevices.get(routeId);
    ...
    private void publishRoutesInMainThread() {
        new Handler(getContext().getMainLooper()).post(this::publishRoutes);
    }

    private void publishRoutes() {
        MediaRouteProviderDescriptor.Builder builder = new MediaRouteProviderDescriptor.Builder();

        // Snapshot the values to avoid ConcurrentModificationException if the map is
        // updated by an NSD callback while we iterate.
        List<CastDevice> snapshot = new ArrayList<>(castDevices.values());

        for (CastDevice castDevice : snapshot) {
