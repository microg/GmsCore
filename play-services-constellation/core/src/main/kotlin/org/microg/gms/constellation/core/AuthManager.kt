...
class AuthManager private constructor(context: Context) {
    ...
    fun getIidToken(projectNumber: String? = null): String {
        ...
        val token = InstanceID.getInstance(context).getToken(sender, "GCM")
        if (token.isNullOrEmpty()) {
            throw IllegalStateException("IID token was empty for sender $sender")
        }
        return token
    }
    ...
    fun getOrCreateKeyPair(): KeyPair {
        ...
        if (privateKeyStr != null && publicKeyStr != null) {
            try {
                ...
                ConstellationStateStore.setPublicKeyAcked(context, false)
            }
        }
